...
import { type SwarmState, type PheromoneState } from "../memory/schemas";
...
export class PheromoneManager {
  ...
  public updatePheromones(swarm: SwarmState, room: Room): void {
    ...
    // Apply decay
    for (const key of Object.keys(pheromones) as Array<keyof PheromoneState>) {
  ...
  public applyDiffusion(rooms: Map<string, SwarmState>): void {
    ...
    for (const [roomName, swarm] of rooms) {
      ...
      for (const neighborName of neighbors) {
        ...
        const diffusibleTypes: Array<keyof PheromoneState> = ["defense", "war", "expand", "siege"];
