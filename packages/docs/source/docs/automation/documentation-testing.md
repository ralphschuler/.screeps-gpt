...
The `docs-pages.yml` workflow includes multiple validation stages:

1. **Theme Validation**: Verifies the Hexo Cactus theme is present before build
2. **Build**: Generates documentation site from source
3. **Build Validation**: Checks that key HTML files exist and are not empty (0 bytes)
4. **Deploy**: Deploys to GitHub Pages
5. **E2E Validation**: Runs E2E tests against deployed site to verify accessibility and functionality

If any validation step fails, the workflow reports the failure and exits, preventing deployment of broken documentation.

#### Regression Prevention Measures

The workflow includes safeguards to prevent common issues:

- **Theme presence check**: Validates theme layout directory exists after clone
- **Post-build validation**: Checks key pages (index, docs, automation, operations) are not empty
- **File size reporting**: Reports actual file sizes to detect 0-byte file generation
- **Early failure**: Fails fast if theme or build validation fails, preventing bad deployments
...
**"Generated blank or minimal index.html"** or **"Empty HTML files (0 bytes)"**

- **Root Cause**: Hexo generates 0-byte HTML files when the theme is not present
- **Solution**: Ensure the Cactus theme is cloned in `packages/docs/themes/cactus/`
- **Verify Theme**: Check that `packages/docs/themes/cactus/layout/` directory exists
- **Manual Fix**: Run `git clone https://github.com/probberechts/hexo-theme-cactus.git packages/docs/themes/cactus`
- **Automated Fix**: The workflow now includes validation to catch this issue early
