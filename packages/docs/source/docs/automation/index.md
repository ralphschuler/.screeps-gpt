---
title: Automation Guides
date: 2025-11-17T10:00:00.000Z
layout: page
---

# Automation Guides

Comprehensive documentation for the repository's GitHub Actions workflows, Copilot automation, and CI/CD pipeline.

## Available Guides

### Core Automation

- [**Automation Overview**](overview.html) - Complete overview of GitHub workflows and Copilot CLI integration
- [**Autonomous Monitoring**](autonomous-monitoring.html) - Strategic analysis and PTR monitoring systems
- [**Label System**](label-system.html) - Standardized three-tier label taxonomy

### Development Tools

- [**Modular Deployment**](modular-deployment.html) - Automated deployment workflows and practices
- [**Push Notifications**](push-notifications.html) - Real-time alerts for CI/CD events
- [**Prompt Audit**](prompt-audit.html) - Copilot prompt management and maintenance

### Best Practices

- [**Semantic Versioning Guide**](semantic-versioning-guide.html) - Version management and changelog automation

## Related Documentation

- [Operations Documentation](../operations/index.html) - Troubleshooting and operational guides
- [Runtime Documentation](../runtime/) - Runtime behavior and internals
- [Security Documentation](../security/) - Security practices and dependency management

## Quick Links

- [GitHub Workflows](https://github.com/ralphschuler/.screeps-gpt/tree/main/.github/workflows)
- [Copilot Prompts](https://github.com/ralphschuler/.screeps-gpt/tree/main/.github/copilot/prompts)
- [Main Documentation Index](../index.html)
