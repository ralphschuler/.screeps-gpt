 ...
 *
 * Note: Builders do NOT pick up dropped energy during gather state to avoid
...
export class BuilderController extends BaseRoleController<BuilderMemory> {
  ...
  public execute(creep: CreepLike): string {
    ...
    const validatedCreep = asCreep(creep, "BuilderController");
    ...
    if (currentState === "gather") {
      ...
      // Note: We intentionally do NOT pick up dropped energy during gather state.
      ...
      // Issue #1501/#1504: This fix ensures builders fill to capacity before building.
      ...
      const target =
        repairTargets.length > 0
          ? (creep.pos.findClosestByPath(repairTargets, { ignoreCreeps: true }) ?? repairTargets[0])
          : null;
      if (target) {
