/**
 * Remote Builder Role Controller
 *
 * Remote builders are responsible for:
 * - Traveling to remote/integration rooms
 * - Mining energy from sources in the remote room
 * - Building construction sites (prioritized: spawns, extensions, containers)
 * - Upgrading the remote room's controller when no construction sites remain
 */
...
const REMOTE_BUILDER_TRAVEL_TASK = "travel" as const;
const REMOTE_BUILDER_GATHER_TASK = "gather" as const;
const REMOTE_BUILDER_BUILD_TASK = "build" as const;

type RemoteBuilderTask =
  | typeof REMOTE_BUILDER_TRAVEL_TASK
  | typeof REMOTE_BUILDER_GATHER_TASK
  | typeof REMOTE_BUILDER_BUILD_TASK;

interface RemoteBuilderMemory extends CreepMemory {
  role: "remoteBuilder";
  task: RemoteBuilderTask;
  version: number;
  homeRoom: string;
  targetRoom: string;
  sourceId?: Id<Source>;
...
export class RemoteBuilderController extends BaseRoleController<RemoteBuilderMemory> {
  public constructor() {
    const config: RoleConfig<RemoteBuilderMemory> = {
      ...
      createMemory: () => ({
        role: "remoteBuilder",
        task: REMOTE_BUILDER_TRAVEL_TASK,
        version: 1,
        homeRoom: "",
        targetRoom: ""
      })
  ...
  public execute(creep: CreepLike): string {
    ...
    if (task === REMOTE_BUILDER_TRAVEL_TASK) {
      ...
      // Arrived in target room - start gathering or building based on energy
      if (creep.store.getUsedCapacity(RESOURCE_ENERGY) > 0) {
        memory.task = REMOTE_BUILDER_BUILD_TASK;
      } else {
        memory.task = REMOTE_BUILDER_GATHER_TASK;
      }
    }

    // Gather energy in the remote room
    if (memory.task === REMOTE_BUILDER_GATHER_TASK) {
      comm?.say(creep, "gather");

      if (creep.store.getFreeCapacity(RESOURCE_ENERGY) === 0) {
        memory.task = REMOTE_BUILDER_BUILD_TASK;
        return REMOTE_BUILDER_BUILD_TASK;
      }

      // Priority 1: Pick up dropped energy
      if (tryPickupDroppedEnergy(creep)) {
        return REMOTE_BUILDER_GATHER_TASK;
      }
    ...
    // No construction sites - upgrade the controller instead
  ...
  private ensureTask(memory: RemoteBuilderMemory): RemoteBuilderTask {
    if (
      memory.task !== REMOTE_BUILDER_TRAVEL_TASK &&
      memory.task !== REMOTE_BUILDER_GATHER_TASK &&
      memory.task !== REMOTE_BUILDER_BUILD_TASK
    ) {
      memory.task = REMOTE_BUILDER_TRAVEL_TASK;
  ...
  private resolveRemoteSource(creep: CreepLike, memory: RemoteBuilderMemory): Source | null {
