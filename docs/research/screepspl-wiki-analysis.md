...
- **Solutions from Wiki**:
  ...
  ```javascript
  // Correct pattern
  Game.spawns["Spawn1"].spawnCreep([WORK, CARRY, MOVE], "CreepName", { memory: { role: "harvester", room: "W1N1" } });
...
function generateBody(role: string, energy: number): BodyPartConstant[] {
  ...
  const maxRepeats = Math.min(Math.floor(energy / unitCost), Math.floor(MAX_CREEP_SIZE / baseUnit.length));
