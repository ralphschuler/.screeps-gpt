...
- Auto-Respawn: The `screeps-autospawner` action checks spawn status after deployment. If the bot is already active (status: "normal"), it exits early with no action. If the bot needs respawning (status: "lost" or "empty"), manual intervention is required through the Screeps web interface.
