...
2. **`post-merge-release.yml`** – Fires on `push` to `main` (excludes release PR merges). It applies lint/format fixes with write access, bumps the patch version via `npm run version:bump`, commits to a release branch, creates a `v*` tag, opens a PR to main, and triggers deployment via workflow chaining.
3. **`deploy.yml`** – Listens for tags that match `v*` OR runs when Post Merge Release completes successfully. It builds the bundle and executes `npm run deploy` to push the code to Screeps using API credentials stored in repository secrets (supports `SCREEPS_DEPLOY_DRY_RUN` for local workflow tests).
