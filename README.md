...
2. **`post-merge-release.yml`** – Fires on `push` to `main`. It applies lint/format fixes with write access, bumps the patch version via `npm run version:bump`, commits the result, and creates a new `v*` tag for release automation.
3. **`deploy.yml`** – Listens for tags that match `v*`. It builds the bundle and executes `npm run deploy` to push the code to Screeps using API credentials stored in repository secrets (supports `SCREEPS_DEPLOY_DRY_RUN` for local workflow tests).
