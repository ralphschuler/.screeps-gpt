...
describe("Project configuration validator script", () => {
  const scriptPath = join(process.cwd(), "scripts", "validate-project-config.ts");

  it("should exist and be readable", () => {
    expect(() => accessSync(scriptPath)).not.toThrow();
  });

  it("should have correct shebang for tsx", () => {
    const content = readFileSync(scriptPath, "utf-8");
    expect(content).toMatch(/^#!\/usr\/bin\/env tsx/);
  });

  it("should import from child_process for Node compatibility", () => {
    const content = readFileSync(scriptPath, "utf-8");
    expect(content).toContain('import { spawnSync } from "child_process"');
  });

  it("should have main validation function", () => {
    const content = readFileSync(scriptPath, "utf-8");
  ...
  it("should check for GitHub token", () => {
    const content = readFileSync(scriptPath, "utf-8");
    expect(content).toContain("function checkGitHubToken()");
    expect(content).toContain("GITHUB_TOKEN");
  });

  it("should validate project access via GraphQL", () => {
    const content = readFileSync(scriptPath, "utf-8");
  ...
  it("should list projects for owner", () => {
    const content = readFileSync(scriptPath, "utf-8");
