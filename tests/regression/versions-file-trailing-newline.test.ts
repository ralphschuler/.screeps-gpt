...
describe("versions.md format requirements", () => {
  it("should end with trailing newline to satisfy versions:update script", () => {
    const versionsPath = resolve(__dirname, "../../packages/docs/source/docs/changelog/versions.md");
    const content = readFileSync(versionsPath, "utf-8");

    // File should end with newline
    expect(content).toMatch(/\n$/);

    // Should have the expected content structure (may have Hexo front matter at start)
    expect(content).toContain("# Release History");
    expect(content).toContain("_This file is generated by `bun run versions:update`");
  });
});
